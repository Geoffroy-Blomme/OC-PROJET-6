...
function filtersListTextContent() {
  const children = filterList.children;
  for (let i = 0; i < children.length; i++) {
    const dataSetValue = children[i].dataset.value;
    children[i].textContent = dataSetValue;
    children[i].setAttribute("aria-label", dataSetValue);
  }
}
...
// Va rendre les filtres "accessibles" c'est à dire cliquables avec des touches du clavier
function makeFiltersAccessible(evt) {
  if (evt.key === "Enter") {
...
function eventListeners() {
  ...
  for (let i = 0; i < filterListChildren.length; i++) {
    ...
    // On rend les filtres accessibles
    filterListChildren[i].addEventListener("keydown", makeFiltersAccessible);
  }
}

function init() {
  filtersListTextContent();
  eventListeners();
}
