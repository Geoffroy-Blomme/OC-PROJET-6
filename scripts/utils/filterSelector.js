...
function swapPlaceClickedAndFirst(evt){
    ...
    let indexClickedElement = getIndexClickedElement(evt.currentTarget);
    //On verifie que l'element clique se trouve bien dans la liste
    if(indexClickedElement != -1){
        // On interverti les places
        let replacedNode = filterList.replaceChild(evt.currentTarget,filterListFirstElement);
        filterList.insertBefore(replacedNode,filterList.children[indexClickedElement]);
    }
